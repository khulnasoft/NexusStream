...
const checkPathVsKey = () => {
  ...
  for (const file of iterator) {
    ...
    if (!componentKey) {
      err = true;
      console.error(`[!] ${file} has no component key! Either its file name should start with 'common' or it should be in a folder named 'common'! See the docs: https://khulnasoft.com/docs/components/guidelines/#folder-structure`);
    } else {
      const uriParts = file.split("/");
      if (uriParts.length < 2) {
        err = true;
        console.error(`[!] ${file} components should be in folders named as the same with their file names! See the docs: https://khulnasoft.com/docs/components/guidelines/#folder-structure`);
      } else {
        ...
        if (folderName != fileName || fileName != keyName) {
          err = true;
          console.error(`[!] ${file} component folder name, component file name without extension and component key without slug should be the same! See the docs: https://khulnasoft.com/docs/components/guidelines/#folder-structure`);
        }
      }
    }
  }
