...
function main() {
	...
	function write_api_key() {
		local key="$1";
		local config_dir="$HOME/.config/nexusstream";
		mkdir -p "$config_dir";
	...
	if test -v "$api_key_name"; then {
		...
		log info "Get your Nexusstream API key from:" \
							link "https://khulnasoft.com/settings/account > Programmatic Access" && printf '\n\n';

		local api_key && read -rs -N 32 -p ">> Paste your nexusstream API key: " api_key && printf '\n';
