...
Nexusstream exposes a limited API for retrieving workflow events, and plans to expose APIs for creating, updating, and deleting workflows programmatically in the future. If you'd like to see this, please upvote and add detail on your use case on [this GitHub issue](https://github.com/KhulnaSoft/nexusstream/issues/8735).

## Base URL

The base URL for all requests is [https://api.khulnasoft.com/v1](https://api.khulnasoft.com/v1).

## Authentication

You authenticate to the REST API using your [Nexusstream API
key](/api/auth/#nexusstream-api-key). When you make API requests, pass an
...
```shell
curl https://api.khulnasoft.com/v1/components \
  -H "Authorization: Bearer <api_key>" \
  -H "Content-Type: application/json" \
  -d '{"component_url": "https://github.com/KhulnaSoft/nexusstream/blob/master/components/rss/sources/new-item-in-feed/new-item-in-feed.js"}'
```
...
```shell
curl https://api.khulnasoft.com/v1/users/me/sources\?limit\=3\&after\=ZGNfSzB1QWVl \
...
Nexusstream uses conventional HTTP response codes to indicate the success or
...
omitted). Codes in the **5xx** range indicate an error with Nexusstream’s server.
...
The full code for a [Nexusstream component](/components/api/).
...
```shell
curl https://api.khulnasoft.com/v1/components/my-component \
  -H "Authorization: Bearer <api_key>"
```
...
Nexusstream operates a global registry of all public components (for example, for
...
```shell
curl https://api.khulnasoft.com/v1/components/registry/github-new-repository \
  -H "Authorization: Bearer <api_key>"
```
...
curl -X DELETE \
  -H "Authorization: Bearer <api key>" \
  "https://api.khulnasoft.com/v1/sources/dc_abc123/events?start_id=1589486981598-0"
```
...
curl -X DELETE \
  -H "Authorization: Bearer <api key>" \
  "https://api.khulnasoft.com/v1/sources/dc_abc123/events?end_id=1589486981598-0"
```
...
curl -X DELETE \
  -H "Authorization: Bearer <api key>" \
  "https://api.khulnasoft.com/v1/sources/dc_abc123/events?start_id=1589486981598-0&end_id=1589486981598-0"
```
...
```shell
curl 'https://api.khulnasoft.com/v1/workspaces/o_abc123/subscriptions' \
  -H 'Authorization: Bearer <api_key>'
```
...
```shell
curl 'https://api.khulnasoft.com/v1/orgs/o_abc123/sources' \
  -H 'Authorization: Bearer <api_key>'
```
  ...
  "data": [
    {
      ...
      "configured_props": {
        "http": {
          "endpoint_url": "https://myendpoint.m.nexusstream.net"
        }
      },
      "active": true,
...
webhooks, emitting those events for use on Nexusstream. Event sources can function
...
```shell
curl 'https://api.khulnasoft.com/v1/users/me/sources' \
  -H 'Authorization: Bearer <api_key>'
```
...
curl https://api.khulnasoft.com/v1/sources \
  -H "Authorization: Bearer <api_key>" \
  -H "Content-Type: application/json" \
  -d '{"component_url": "https://github.com/KhulnaSoft/nexusstream/blob/master/components/rss/sources/new-item-in-feed/new-item-in-feed.js", "name": "your-name-here", "configured_props": { "url": "https://rss.m.nexusstream.net", "timer": { "intervalSeconds": 60 }}}'
```
...
The full code for a [Nexusstream
...
`https://khulnasoft.com/@dylan/example-rss-sql-workflow-p_2gCPml/edit`.
...
Nexusstream also exposes channels for logs and errors:
...
**Currently, this feature is enabled only on the API. The Nexusstream UI will not
...
```shell
curl "https://api.khulnasoft.com/v1/auto_subscriptions?event_name=$errors&listener_id=p_abc123" \
  -X POST \
  -H "Authorization: Bearer <api_key>" \
  -H "Content-Type: application/json"
```
...
Nexusstream supports webhooks as a way to deliver events to a endpoint you own.
...
Nexusstream will deliver those tweets directly to your webhook's URL without
running a workflow.
...
`https://endpoint.m.nexusstream.net` using the following command:
...
```json
{
  "data": {
    ...
    "name": null,
    "description": null,
    "url": "https://endpoint.m.nexusstream.net",
    "active": true,
...
```shell
curl "https://api.khulnasoft.com/v1/webhooks/wh_abc123" \
  -X DELETE \
  -H "Authorization: Bearer <api_key>" \
  -H "Content-Type: application/json"
```
...
```shell
curl 'https://api.khulnasoft.com/v1/workflows/p_abc123/event_summaries?expand=event&limit=1' \
  -H 'Authorization: Bearer <api_key>'
```
...
```shell
curl 'https://api.khulnasoft.com/v1/workflows/p_abc123/$errors/event_summaries?expand=event&limit=1' \
  -H 'Authorization: Bearer <api_key>'
```
    ...
    "orgs": [
      {
        ...
        "id": "o_abc123",
        "orgname": "myworkspace",
        "email": "workspace@khulnasoft.com",
        "daily_credits_quota": 100,
        "daily_credits_used": 0
      },
      {
...
```shell
curl 'https://api.khulnasoft.com/v1/users/me/subscriptions' \
  -H 'Authorization: Bearer <api_key>'
```
...
```shell
curl 'https://api.khulnasoft.com/v1/users/me/webhooks' \
  -H 'Authorization: Bearer <api_key>'
```
...
```json
{
  ...
  "data": [
    ...
    },
    {
      ...
      "url": "https://endpoint2.m.nexusstream.net",
      "active": true,
