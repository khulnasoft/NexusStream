...
For a component to be accepted into the Nexusstream registry, it should follow these guidelines below. These guidelines help ensure components are high quality, are intutive for both Nexusstream users and component developers to use and extend.

::: tip Questions about best practices?

Join the discussion with fellow Nexusstream component developers at the [#contribute channel](https://nexusstream-users.slack.com/archives/C01E5KCTR16) in Slack or [on Discourse](https://khulnasoft.com/community/c/dev/11).
...
webhook](https://github.com/khulnasoft/nexusstream/blob/master/components/github/sources/custom-webhook-events)
...
When publishing components to the Nexusstream registry, the `key` must be unique
...
Nexusstream registry components try to follow [semantic versioning](https://semver.org/). From their site:
...
directory](https://github.com/khulnasoft/nexusstream/tree/master/components).
...
Each app should have a `package.json` in its root folder. If one doesn't exist, run `npm init` in the app's root folder and customize the file using [this `package.json`](https://github.com/KhulnaSoft/nexusstream/blob/55236b3aa993cbcb545e245803d8654c6358b0a2/components/stripe/package.json) as a template.
...
Here's an example of an [app `README.md` within the `discord` component on the Nexusstream registry](https://github.com/KhulnaSoft/nexusstream/blob/master/components/discord/README.md). That same content is rendered within the [Nexusstream integration page for the Discord app](https://khulnasoft.com/apps/discord).
...
- Be encapsulated as a [generator](https://mzl.la/37z6Sh6) so that the component
  ...
  methods](https://github.com/KhulnaSoft/nexusstream/tree/master/components/microsoft_onedrive/microsoft_onedrive.app.mjs)
  to list files.
...
- Persist the last page number, token or record ID right after processing, so
  ...
  component](https://github.com/KhulnaSoft/nexusstream/tree/master/components/microsoft_onedrive/sources/new-file/new-file.mjs)
  for an example.
...
out](https://khulnasoft.com/community/c/dev/11).
...
Prop definitions will also be surfaced for apps the Nexusstream marketplace.
...
surfaced in documentation for apps in the Nexusstream marketplace. For example:
...
Nexusstream does not currently support unit tests to validate that changes to app
...
option of customizing the code for components within Nexusstream. When using this
...
See [Google
Drive](https://github.com/KhulnaSoft/nexusstream/tree/master/components/google_drive) for an
...
[AWS
sources](https://github.com/KhulnaSoft/nexusstream/tree/master/components/aws)
...
- The async option to select an Airtable Base is self-explanatory so includes no
  ...
  ![image-20210326151557417](https://res.cloudinary.com/nexusstreamin/image/upload/v1672810770/ixb3aozdijmz0zfqxmvy.png)
...
recency and popularity. However, the best default for the use case on Nexusstream
is recency.
...
[Hubspot](https://github.com/KhulnaSoft/nexusstream/blob/a9b45d8be3b84504dc22bb2748d925f0d5c1541f/components/hubspot/hubspot.app.mjs#L136)
for an example of offset-based pagination. See
[Twitter](https://github.com/KhulnaSoft/nexusstream/blob/d240752028e2a17f7cca1a512b40725566ea97bd/components/twitter/twitter.app.mjs#L200)
for an example of cursor-based pagination.

### Dynamic props

[Dynamic props](/components/api/#dynamic-props) can improve the user experience for components. They let you render props in the Nexusstream dynamically, based on the value of other props, and can be used to collect more specific information that can make it easier to use the component. See the Google Sheets example in the linked component API docs.
...
Mentions](https://github.com/KhulnaSoft/nexusstream/blob/master/components/twitter/sources/search-mentions/search-mentions.mjs#L83-L88)
...
should not include the app name. NOTE: Nexusstream does not currently distinguish
...
- Polling sources should always emit events on the first run (see the [Spotify: New Playlist](https://github.com/KhulnaSoft/nexusstream/blob/master/components/spotify/sources/new-playlist/new-playlist.mjs) source as an example)
- Webhook-based sources should attempt to fetch existing events in the `deploy()` hook during source creation (see the [Jotform: New Submission](https://github.com/KhulnaSoft/nexusstream/blob/master/components/jotform/sources/new-submission/new-submission.mjs) source)
...
2. Add a new file called `test-event.mjs` in the same directory as the component source and export the JSON event via `export default` ([example](https://github.com/KhulnaSoft/nexusstream/blob/master/components/jotform/sources/new-submission/test-event.mjs))
3. In the source component code, make sure to import that file as `sampleEmit` ([example](https://github.com/KhulnaSoft/nexusstream/blob/master/components/jotform/sources/new-submission/new-submission.mjs#L2))
4. And finally, export the `sampleEmit` object ([example](https://github.com/KhulnaSoft/nexusstream/blob/master/components/jotform/sources/new-submission/new-submission.mjs#L96))

This will render a "Generate Test Event" button in the UI for users to emit that sample event:

![generate-sample-event](https://res.cloudinary.com/nexusstreamin/image/upload/v1690488844/generate-test-event_drjykm.gif)
...
For polling sources in the Nexusstream registry, the default polling interval is set as a global config. Individual sources can access that default within the props definition:
...
[Hooks](/components/api/#hooks) are methods that are automatically invoked by Nexusstream
...
Instead, [use `@pipedream/platform` axios](/nexusstream-axios/).
...
Airtable](https://github.com/KhulnaSoft/nexusstream/blob/cb4b830d93e1495d8622b0c7dbd80cd3664e4eb3/components/airtable/actions/common-list.js#L48-L63).
...
[`maxRequests`
prop](https://github.com/KhulnaSoft/nexusstream/blob/cb4b830d93e1495d8622b0c7dbd80cd3664e4eb3/components/twitter/twitter.app.mjs#L52))
to avoid exceeding Twitter's rate limits. [See the Airtable
components](https://github.com/KhulnaSoft/nexusstream/blob/e2bb7b7bea2fdf5869f18e84644f5dc61d9c22f0/components/airtable/airtable.app.js#L14)
