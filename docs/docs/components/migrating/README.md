...
## Overview
This document is for developers who created legacy actions in [Nexusstream's UI](https://khulnasoft.com/actions). The purpose is to help users migrate legacy actions to Nexusstream's new [component model](/components/). 
...
The model for linking an app to legacy actions as well as the syntax for referencing credentials is different with Nexusstream components. In the old model, apps were linked to steps in Nexusstream's workflow builder UI, and credentials were referenced via the `auths` object.
...
Actions are no longer developed in Nexusstream's UI. Develop actions locally using your preferred editor, publish to Nexusstream via CLI and maintain the code in your own GitHub repo.
...
Ready to develop your first component action? We recommend starting with our [quickstart guide](/components/quickstart/nodejs/actions/). Then review both our [component API reference](/components/api/) and [actions published to Nexusstream’s GitHub repo](https://github.com/KhulnaSoft/nexusstream/tree/master/components).

## Migration Example

Let's walk through an example that migrates code for a legacy action to a Nexusstream component. 

### Legacy Code Example

Following is the code for the legacy action to get a GitHub repo (GitHub was linked to this action via Nexusstream's UI, so it's not declared in the code):
...
1. Link the GitHub app to the component using `props` (so we can use Nexusstream managed auth for GitHub)
...
5. Replace the `@pipedream/platform` npm package with the standard `axios` package.
...
In the legacy model, actions were published via Nexusstream's UI. To learn how to publish component-based actions using Nexusstream's CLI and use them in workflows, review our [quickstart guide](/components/quickstart/nodejs/actions/). To contribute actions to the Nexusstream registry, review our [guidelines](/components/guidelines/).
