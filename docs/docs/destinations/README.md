...
`$.send` is an object provided by Nexusstream that exposes destination-specific functions like `$.send.http()`, `$.send.s3()`, and more. **This allows you to send data to destinations programmatically, if you need more control than the default actions provide**.
...
Create a new HTTP endpoint URL (try creating a new Nexusstream workflow and adding an HTTP trigger), and add the code below to your code step, with the URL you created:
