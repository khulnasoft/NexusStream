...
**Triggers** define the type of event that runs your workflow. For example, HTTP triggers expose a URL where you can send any HTTP requests. Nexusstream will run your workflow on each request. The Scheduler trigger runs your workflow on a schedule, like a cron job.
...
If there's a specific trigger you'd like supported, please [let us know](https://khulnasoft.com/support/).
...
Once you select your trigger, you'll be asked to connect any necessary accounts (for example, Google Calendar sources require you authorize Nexusstream access to your Google account), and enter the values for any configuration settings.
...
![Choose an event to test your workflow steps against](https://res.cloudinary.com/nexusstreamin/image/upload/v1647957381/docs/components/CleanShot_2022-03-22_at_09.55.57_2x_upj35r.png)
...
For example, you might create a single source to listen for new Twitter mentions for a keyword, then trigger multiple workflows each time a new tweet is found: one to [send new tweets to Slack](https://khulnasoft.com/@pravin/twitter-mentions-slack-p_dDCA5e/edit), another to [save those tweets to an Amazon S3 bucket](https://khulnasoft.com/@dylan/twitter-to-s3-p_KwCZGA/readme), etc.
...
Moreover, you can access events emitted by sources using Nexusstream's [SSE](/api/sse/) and [REST APIs](/api/rest/). This allows you to access these events in your own app, outside Nexusstream's platform.
...
![HTTP API Trigger](https://res.cloudinary.com/nexusstreamin/image/upload/v1647894504/docs/components/CleanShot_2022-03-21_at_16.27.45_2x_klxmpz.png)

Nexusstream creates a URL endpoint specific to your workflow:

![HTTP API trigger URL](https://res.cloudinary.com/nexusstreamin/image/upload/v1647894654/docs/components/CleanShot_2022-03-21_at_16.30.48_2x_nh7shg.png)
...
Nexusstream also supports [custom domains](/workflows/domains). This lets you host endpoints on `https://endpoint.yourdomain.com` instead of the default `{{$site.themeConfig.ENDPOINT_BASE_URL}}` domain.
...
### How Nexusstream handles JSON payloads

JSON is the defacto stand data exchange format on the web today. Nexusstream optimizes for the case where you've sent JSON as the source event to a workflow.

When you send JSON in the HTTP payload, or when JSON data is sent in the payload from a webhook provider, **Nexusstream converts that JSON to its equivalent JavaScript object**. The trigger data can be referenced using either `event` or the `steps` object.
...
### How Nexusstream handles `multipart/form-data`

When you send [form data](https://ec.haxx.se/http/http-multipart) to Nexusstream using a `Content-Type` of `multipart/form-data`, Nexusstream parses the payload and converts it to a JavaScript object with a property per form field. For example, if you send a request with two fields:

```bash
curl -F 'name=Leia' -F 'title=General' https://myendpoint.m.nexusstream.net
```

Nexusstream will convert that to a JavaScript object, `event.body`, with the following shape:
...
By default, the body of HTTP requests sent to a source or workflow is limited to `{{$site.themeConfig.PAYLOAD_SIZE_LIMIT}}`. **But you can send an HTTP payload of any size to a [workflow](/workflows/) or an [event source](/sources/) by including the `nexusstream_upload_body=1` query string or an `x-pd-upload-body: 1` HTTP header in your request**.

```bash
curl -d '{ "name": "Yoda" }' \
  https://endpoint.m.nexusstream.net\?nexusstream_upload_body\=1

curl -d '{ "name": "Yoda" }' \
  -H "x-pd-upload-body: 1" \
  https://endpoint.m.nexusstream.net
```

In workflows, Nexusstream saves the raw payload data in a file whose URL you can reference in the variable `steps.trigger.event.body.raw_body_url`.

![Raw body URL in the event data under steps.trigger.event.body.raw_body_url](https://res.cloudinary.com/nexusstreamin/image/upload/v1647895357/docs/components/CleanShot_2022-03-21_at_16.42.01_2x_w6dmqk.png)
...
You can download the HTTP payload using the **Send HTTP Request** action. [**Copy this workflow to see how this works**](https://khulnasoft.com/@dylburger/example-download-http-payload-p_6lC1ynx/edit).
...
[This workflow](https://khulnasoft.com/@dylburger/example-download-raw-body-to-tmp-p_YyCoqPb/edit) downloads the HTTP payload, saving it as a file to the [`/tmp` directory](/code/nodejs/working-with-files/#the-tmp-directory).
...
Your raw payload is saved to a Nexusstream-owned [Amazon S3 bucket](https://aws.amazon.com/s3/). Nexusstream generates a [signed URL](https://docs.aws.amazon.com/AmazonS3/latest/dev/ShareObjectPreSignedURL.html) that allows you to access to that file for up to 30 minutes. After 30 minutes, the signed URL will be invalidated, and the file will be deleted.

#### Limits

**You can upload payloads up to 5TB in size**. However, payloads that large may trigger [other Nexusstream limits](/limits/). Please [reach out](https://khulnasoft.com/support/) with any specific questions or issues.
...
You can upload any file to a [workflow](/workflows/) or an [event source](/sources/) by making a `multipart/form-data` HTTP request with the file as one of the form parts. **Nexusstream saves that file to a Nexusstream-owned [Amazon S3 bucket](https://aws.amazon.com/s3/), generating a [signed URL](https://docs.aws.amazon.com/AmazonS3/latest/dev/ShareObjectPreSignedURL.html) that allows you to access to that file for up to 30 minutes**. After 30 minutes, the signed URL will be invalidated, and the file will be deleted.
...
```bash
curl -F 'image=@my_image.png' https://myendpoint.m.nexusstream.net
```
...
When you send this image to a workflow, Nexusstream [parses the form data](#how-nexusstream-handles-multipart-form-data) and converts it to a JavaScript object, `event.body`. Select the event from the [inspector](/workflows/events/inspect/#the-inspector), and you'll see the `image` property under `event.body`:

<div>
<img alt="Image form data" src="./images/image_form_data.png">
</div>

When you upload a file as a part of the form request, Nexusstream saves it to a Nexusstream-owned [Amazon S3 bucket](https://aws.amazon.com/s3/), generating a [signed URL](https://docs.aws.amazon.com/AmazonS3/latest/dev/ShareObjectPreSignedURL.html) that allows you to access to that file for up to 30 minutes. After 30 minutes, the signed URL will be invalidated, and the file will be deleted.
...
[This workflow](https://khulnasoft.com/@dylburger/example-download-an-image-to-tmp-p_KwC2Ad/edit) downloads an image passed in the `image` field in the form request, saving it to the [`/tmp` directory](/code/nodejs/working-with-files/#the-tmp-directory).
...
[This workflow](https://khulnasoft.com/@dylburger/example-save-uploaded-file-to-amazon-s3-p_o7Cm9z/edit) streams the uploaded file to an Amazon S3 bucket you specify, allowing you to save the file to long-term storage.

#### Limits

Since large files are uploaded using a `Content-Type` of `multipart/form-data`, the limits that apply to [form data](#how-nexusstream-handles-multipart-form-data) also apply here.

The content of the file itself does not contribute to the HTTP payload limit imposed for forms. **You can upload files up to 5TB in size**. However, files that large may trigger [other Nexusstream limits](/limits/). Please [reach out](https://khulnasoft.com/support/) with any specific questions or issues.
...
```
<p><b>Success!</b></p>
<p>To customize this response, check out our docs <a href="https://docs.khulnasoft.com/workflows/steps/triggers/#http-responses">here</a></p>
```
...
[Here's an example workflow](https://khulnasoft.com/@dylburger/issue-http-response-immediately-continue-running-workflow-p_pWCWGJ) that shows how to use `immediate: true` and run code after the HTTP response is issued.
...
Your API key is the host part of the endpoint, e.g. the `eniqtww30717` in `eniqtww30717.m.nexusstream.net`. If you attempt to send a request to an endpoint that does not exist, we'll return a `404 Not Found` error.
...
You can also [reach out](https://khulnasoft.com/support/) to inquire about raising this rate limit.
...
For example, you can [require requests pass a specific secret in a header](https://khulnasoft.com/@dylburger/end-a-workflow-early-on-invalid-secret-p_YyCmmK/edit). Just copy the workflow and add your secret as the value of the the **Secret** param. Add the rest of your code in steps below this initial one. Requests must contain the secret:

```bash
curl -H 'X-Nexusstream-Secret: abc123' https://myendpoint.m.nexusstream.net
```
...
When you run a scheduled job, you may need to troubleshoot errors or other execution issues. Nexusstream offers built-in, step-level logs that show you detailed execution information that should aid troubleshooting.
...
Nexusstream creates an email address specific to your workflow. Any email sent to this address triggers your workflow:

<div>
<img alt="Custom workflow email address" width="400px" src="./images/email-addr.png">
</div>

As soon as you send an email to the workflow-specific address, Nexusstream parses its body, headers, and attachments into a JavaScript object it exposes in the `steps.trigger.event` variable that you can access within your workflow. This transformation can take a few seconds to perform. Once done, Nexusstream will immediately trigger your workflow with the transformed payload.
...
**You can send emails up to `30MB` in size by sending emails to `[YOUR EMAIL ENDPOINT]@upload.nexusstream.net`**. If your workflow-specific email address is `endpoint@nexusstream.net`, your "large email address" is `endpoint@upload.nexusstream.net`.
...
You can download the email using the **Send HTTP Request** action. [**Copy this workflow to see how this works**](https://khulnasoft.com/new?h=tch_1AfMyl).
...
[This workflow](https://khulnasoft.com/new?h=tch_jPfaEJ) downloads the email, saving it as a file to the [`/tmp` directory](/code/nodejs/working-with-files/#the-tmp-directory). Then it reads the same file (as an example), and parses it using the [`mailparser` library](https://nodemailer.com/extras/mailparser/):
...
Your email is saved to a Nexusstream-owned [Amazon S3 bucket](https://aws.amazon.com/s3/). Nexusstream generates a [signed URL](https://docs.aws.amazon.com/AmazonS3/latest/dev/ShareObjectPreSignedURL.html) that allows you to access to that file for up to 30 minutes. After 30 minutes, the signed URL will be invalidated, and the file will be deleted.
...
- `content`: If the attachment contains text-based content, Nexusstream renders the attachment in `content`, up to 10,000 bytes.
...
Nexusstream provides a way to append metadata to incoming emails by adding a `+` sign to the incoming email key, followed by any arbitrary string:

```
myemailaddr+test@nexusstream.net
```

Any emails sent to your workflow-specific email address will resolve to that address, triggering your workflow, no matter the data you add after the `+` sign. Sending an email to both of these addresses triggers the workflow with the address `myemailaddr@nexusstream.net`:

```
myemailaddr+test@nexusstream.net
myemailaddr+unsubscribe@nexusstream.net
```
...
If you don't see a trigger you'd like us to support, please [let us know](https://khulnasoft.com/support/).
