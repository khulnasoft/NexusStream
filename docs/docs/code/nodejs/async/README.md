...
thumbnail: https://res.cloudinary.com/nexusstreamin/image/upload/v1646841376/docs/icons/icons8-time-96_kupxpi.png
---
...
**Any asynchronous code within a Node.js code step must complete before the next step runs**. This ensures future steps have access to its data. If Nexusstream detects that code is still running by the time the step completes, you'll see the following warning below the code step:
...
Most Node.js packages that run async code return Promises as the result of method calls. For example, [`axios`](/code/nodejs/http-requests/#basic-axios-usage-notes) is an HTTP client. If you make an HTTP request like this in a Nexusstream code step:
...
and **it will not run**. By running a callback function in this way, we're saying that we want the function to be run asynchronously. But on Nexusstream, this code must finish by the time the step ends. **You can wrap this callback function in a Promise to make sure that happens**. Instead of running:
...
This warning can also be a false positive. If you're successfully awaiting all Promises, Nexusstream could be throwing the warning in error. If you observe this, please [file a bug](https://github.com/KhulnaSoft/nexusstream/issues/new?assignees=&labels=bug&template=bug_report.md&title=%5BBUG%5D+).

Packages that make HTTP requests or read data from disk (for example) fail to resolve Promises at the right time, or at all. This means that Nexusstream is correctly detecting that code is still running, but there's also no issue - the library successfully ran, but just failed to resolve the Promise. You can safely ignore the error if all relevant operations are truly succeeding.
