...
Nexusstream supports [Node.js v{{$site.themeConfig.NODE_VERSION}}](https://nodejs.org/).
...
## How Nexusstream Node.js components work
...
When the step runs, Nexusstream executes the `run` method:
...
- Nexusstream passes the `steps` variable to the run method. `steps` is also an object, and contains the [data exported from previous steps](/workflows/steps/#step-exports) in your workflow.
...
When you [connect an account to a step](/connected-accounts/#from-a-code-step), Nexusstream exposes the auth info in the variable [`this.appName.$auth`](/code/nodejs/auth/#accessing-connected-account-data-with-this-appname-auth).
...
Nexusstream will attempt to catch syntax errors when you're writing code, highlighting the lines where the error occurred in red.
...
To use an npm package on Nexusstream, simply `import` it:
...
When Nexusstream runs your workflow, we download the associated npm package for you before running your code steps.
...
Other packages require access to binaries or system libraries that aren't installed in the Nexusstream execution environment.

If you're seeing any issues with a specific package, please [let us know](https://khulnasoft.com/support/) and we'll try to help you make it work.

### Pinning package versions

Each time you deploy a workflow with Node.js code, Nexusstream downloads the npm packages you `import` in your step. **By default, Nexusstream deploys the latest version of the npm package each time you deploy a change**.
...
- [Use `$send.http()`](/destinations/http/#using-send-http-in-workflows), a Nexusstream-provided method for making asynchronous HTTP requests.
...
![Example of an ConfigurationError](https://res.cloudinary.com/nexusstreamin/image/upload/v1651680315/docs/components/CleanShot_2022-05-04_at_12.04.38_2x_vf8jny.png)
...
Nexusstream supports [environment variables](/environment-variables/) for keeping secrets separate from code. Once you create an environment variable in Nexusstream, you can reference it in any workflow using `process.env.VARIABLE_NAME`. The values of environment variables are private.
...
If you're trying to run code that doesn't work or you have questions about any limits on code steps, [please reach out](https://khulnasoft.com/support/).

## Editor settings

We use the [Monaco Editor](https://microsoft.github.io/monaco-editor/), which powers VS Code and other web-based editors.

We also let you customize the editor. For example, you can enable Vim mode, and change the default tab size for indented code. Visit your [**Settings**](https://khulnasoft.com/settings) to modify these settings.
...
When you're searching for how to do something in JavaScript, some of the code you try might not work in Nexusstream. This could be because the code expects to run in a browser, not a Node.js environment. The same goes for [npm packages](#using-npm-packages).

### I'm new to programming

Many of the most basic JavaScript tutorials are geared towards writing code for a web browser to run. This is great for learning — a webpage is one of the coolest things you can build with code. We recommend starting with these general JavaScript tutorials and trying the code you learn on Nexusstream:
