...
**Anything you can do in Go, you can do in a Nexusstream Workflow**. You can use any of the [Go packages available](https://pkg.go.dev/) with a simple `import` — no `go get` needed.

Nexusstream supports [Go v{{$site.themeConfig.GO_LANG_VERSION}}](https://go.dev) in workflows.

When you write Go code on Nexusstream, you can [share data between steps](/code/bash/#sharing-data-between-steps) and [access environment variables](/code/bash/#using-environment-variables). However, you can't connect accounts, return HTTP responses, or take advantage of other features available in the [Node.js](/code/nodejs/) environment at this time.

If you have any feedback on the Go runtime, please let us know in [our community](https://khulnasoft.com/support).
...
Data from the initial workflow trigger and other steps are available in the `nexusstream-go` package.
...
import (
	"fmt"
	"github.com/KhulnaSoft/nexusstream-go"
)
...
You can leverage any [environment variables defined in your Nexusstream account](/environment-variables/#environment-variables) in a Go step. This is useful for keeping your secrets out of code as well as keeping them flexible to swap API keys without having to update each step individually.
...
func main() {
  ...
  // Send the POST request
  req, err := http.NewRequest("POST", "https://api.twitter.com/2/users/@nexusstream/mentions'", payloadBuf)
