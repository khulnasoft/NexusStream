...
export default {
  ...
  propDefinitions: {
    ...
    subscriptionSource: {
      ...
      [See details here](https://khulnasoft.com/docs/api/rest/#listen-for-events-from-another-source-or-workflow)`,
  ...
  methods: {
    async _makeAPIRequest({
      ...
      opts.headers["user-agent"] = "@KhulnaSoft/nexusstream v0.1";
      const { path } = opts;
      delete opts.path;
      opts.url = `https://api.khulnasoft.com/v1${path[0] === "/"
        ? ""
        : "/"
      }${path}`;
      return axios($, opts);
