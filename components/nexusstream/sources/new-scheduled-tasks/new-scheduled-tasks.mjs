...
export default {
  key: "nexusstream-new-scheduled-tasks",
  ...
  methods: {
    ...
    async selfSubscribe() {
      ...
      if (!isSubscribedToSelf) {
        ...
        console.log(
          await this.nexusstream.subscribe(componentId, componentId, selfChannel),
        );
    ...
    async deleteEvent(event) {
      ...
      const events = await this.nexusstream.listEvents(
        componentId,
        inChannel,
      );
      ...
      // Delete the event
      await this.nexusstream.deleteEvent(
        componentId,
        eventToCancel.id,
        inChannel,
      );
