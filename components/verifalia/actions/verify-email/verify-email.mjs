...
export default {
  ...
  async run({ $ }) {
    ...
    // This component takes advantage of the new Nexusstream's flow suspension: we handle
    ...
    if (run.runs === 1) {
      ...
      // the one we are allowed by Nexusstream, we just let the user know.
      ...
      if ($.context.test) {
        ...
        // See: https://khulnasoft.com/docs/limits/#time-per-execution
        ...
        // on the Nexusstream's new ability to suspend execution flows.
      ...
      try {
        ...
        if (error instanceof OperationCanceledError) {
          // Had to replace the word req*ire with a synonym because of https://github.com/KhulnaSoft/nexusstream/issues/3187 :)

          throw new Error("This operation would need some more time to complete and that would not work properly in " +
                        "Nexusstream's test mode. Please deploy your workflow to get a meaningful email verification result.");
        }
      ...
      if (job.overview.status === "Completed") {
        ...
        // Blocked by https://github.com/KhulnaSoft/nexusstream/issues/3151
