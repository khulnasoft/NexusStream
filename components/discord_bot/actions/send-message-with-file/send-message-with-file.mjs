...
export default {
  ...
  props: {
    ...
    filePath: {
      type: "string",
      label: "File Path",
      description: "The path to the image file saved to the `/tmp` directory (e.g. `/tmp/image.png`). [see docs here](https://khulnasoft.com/docs/workflows/steps/code/nodejs/working-with-files/#the-tmp-directory).",
    },
    message: {
      propDefinition: [
        discord,
  ...
  async run({ $ }) {
    ...
    if (channelId) {
      const createMessageResponse = await this.discord.sendMessageWithFile({
        ...
        content: this.includeSentViaNexusstream
          ? this.appendNexusstreamText(this.message)
          : this.message,
      });
