...
export default {
  ...
  props: {
    ...
    initImage: {
      type: "string",
      label: "Init Image",
      description: "Image used to initialize the diffusion process, in lieu of random noise. It can be an URL to the image or a path to the image file saved to the `/tmp` directory (e.g. `/tmp/image.png`). [see docs here](https://khulnasoft.com/docs/workflows/steps/code/nodejs/working-with-files/#the-tmp-directory).",
    },
    cfgScale: {
      propDefinition: [
        common.props.dreamstudio,
