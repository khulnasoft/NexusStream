...
export default {
  ...
  props: {
    ...
    // eslint-disable-next-line nexusstream/props-description,nexusstream/props-label
    slackApphook: {
      ...
      for (const message of messages) {
        ...
        if (event) {
          if (!event.client_msg_id) {
            event.nexusstream_msg_id = `pd_${Date.now()}_${Math.random().toString(36)
              .substr(2, 10)}`;
          }

          this.$emit(event, {
            id: event.client_msg_id || event.nexusstream_msg_id,
