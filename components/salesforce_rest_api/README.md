...
You can install the Nexusstream Salesforce app in the [Accounts](https://khulnasoft.com/accounts) section of your Nexusstream account, or directly in a workflow.
...
**Label**: Nexusstream API Access
**API Name**: Nexusstream
**Description**: Adds a set of permissions required for Nexusstream. 
...
If you'd like to utilize Nexusstream's webhook triggers, you will need to add the following permissions to the permissions set as well:
- Apex REST Services
- Author Apex
...
If you're having issues connecting your Salesforce account on Nexusstream, please ensure that IP Restrictions are not enabled for Nexusstream. To learn more about what this means, please see the Salesforce documentation [here](https://help.salesforce.com/s/articleView?language=en_US&id=sf.connected_app_continuous_ip.htm&type=5).
...
3. If Nexusstream is not yet installed, click **Install**, otherwise click **Manage App Policies**.
...
If you happen to stumble on the error: `UNKNOWN_EXCEPTION: admin operation already in progress` when creating an **Instant** trigger, you can follow the steps below to use the Salesforce Flow Builder to be able to use webhooks with Nexusstream. This is a known error in Salesforce.

1. Create a New Workflow on [Nexusstream](https://khulnasoft.com) and [add a HTTP trigger](https://khulnasoft.com/docs/workflows/steps/triggers/#http).
...
15. Back to the Nexusstream Workflow, create a new step with the **Salesforce Convert SOAP Object** action.
