...
export default {
  ...
  propDefinitions: {
    ...
    media: {
      type: "string",
      label: "Media",
      description: "The media content of the pin, only images are supported currently, please provide an image url or an image file from `/tmp`. To upload a file to `/tmp` folder, please follow the doc [here](https://khulnasoft.com/docs/code/nodejs/working-with-files/#writing-a-file-to-tmp)",
    },
  },
  methods: {
    _getUrl(path) {
    ...
    _getHeaders(headers = {}) {
      return {
        ...
        "User-Agent": "@KhulnaSoft/nexusstream v0.1",
        ...headers,
      };
    },
    async _makeRequest({
