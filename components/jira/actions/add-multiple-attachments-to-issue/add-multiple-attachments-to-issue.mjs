...
export default {
  ...
  props: {
    ...
    filenames: {
      type: "string[]",
      label: "File names",
      description: "Array of file paths in `/tmp` folder to add as attachments. To upload a file to `/tmp` folder, please follow the [doc here](https://khulnasoft.com/docs/code/nodejs/working-with-files/#writing-a-file-to-tmp)",
    },
  },
  async run({ $ }) {
    const responses = [];
