...
export const componentMaker = (appData: AppData) => {
  const typeDefs = `Here are TypeScript type definitions for Nexusstream components:
...
// https://khulnasoft.com/docs/components/api/#async-options-example
...
// https://khulnasoft.com/docs/components/api/#referencing-values-from-previous-props
...
// https://khulnasoft.com/docs/components/api/#prop-definitions-example
export interface PropDefinitionReference {
  propDefinition: PropDefinition;
}

// https://khulnasoft.com/docs/components/api/#app-props
...
// https://khulnasoft.com/docs/components/api/#user-input-props
...
// https://khulnasoft.com/docs/components/api/#interface-props
export interface InterfaceProp extends BasePropInterface {
  type: "$.interface.http" | "$.interface.timer";
  default?: string | DefaultConfig;
}

// https://khulnasoft.com/docs/code/nodejs/using-data-stores/#using-the-data-store
...
// https://khulnasoft.com/docs/components/api/#run
...
You MUST define an \`async\` options method if an API endpoint exists that can be used to fetch the options for the prop. This allows Nexusstream to make an API call to fetch the options for the prop when the user is configuring the component, rather than forcing the user to enter values for the option manually.
...
`;
  ...
  const nexusstreamPlatformAxiosTypeDefs = `@pipedream/platform axios TypeScript types:
...
${nexusstreamPlatformAxiosTypeDefs}`;
  ...
  if (appData && Object.keys(appData).length > 0) {
    ...
    if (component_code_scaffold_raw) {
      testRequestCode = `Here's an example Nexusstream component that makes a test request against the ${app} API:\n\n${component_code_scaffold_raw}`;
    }
...
First, I need to teach you what a Nexusstream component is. All Nexusstream components are Node.js modules that have a default export: \`defineComponent\`. \`defineComponent\` is provided to the environment as a global — you do not need to import \`defineComponent\`. \`defineComponent\` is a function that takes an object — a Nexusstream component — as its single argument. The object contains a \`props\` property, which defines a single prop of type "app". This lets the user connect their ${app} account to the step, authorizing requests to the ${app} API (${description}). ${authText}. Don't reference the \`steps\` and \`$\` variables passed to the \`run\` method.

${axiosInstructions}

${testRequestCode}

Here's an example Nexusstream component that makes a test request against the Slack API:
...
The code you generate should be placed within the \`run\` method of the Nexusstream component. Use this format:
...
First, I need to teach you what a Nexusstream component is. All Nexusstream components are Node.js modules that have a default export: \`defineComponent\`. \`defineComponent\` is provided to the environment as a global — you do not need to import \`defineComponent\`. \`defineComponent\` is a function that takes an object — a Nexusstream component — as its single argument. 

${axiosInstructions}

Here's an example Nexusstream component that makes a test request using @pipedream/platform axios:
...
The example code should be placed within the \`run\` method of the Nexusstream component.
...
\`return\` the final value from the step. The data returned from steps must be JSON-serializable. The returned data is displayed in the Nexusstream execution logs below the step when the user runs the code.
